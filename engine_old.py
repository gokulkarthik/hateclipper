...
class CLIPClassifier(pl.LightningModule):
    ...
    def __init__(self, args, fine_grained_labels, compute_fine_grained_metrics):
        ...
        if args.head == 'clip':
            ...
            pre_output_layers = [nn.Dropout(p=args.drop_probs[1]), nn.Linear(self.map_dim*2, self.map_dim), nn.ReLU(), nn.Dropout(p=args.drop_probs[2])]
            ...
            if self.fine_grained_labels:
                # self.fine_grained_outputs = {}
                # for fine_grained_label in self.fine_grained_labels:
                #     self.fine_grained_outputs[fine_grained_label] = nn.Linear(self.map_dim, 1)
    ...
    def common_step(self, batch, batch_idx, calling_function):
        ...
        if self.head == 'clip':
            ...
            output['loss'] = (loss_text + loss_image) / 2
